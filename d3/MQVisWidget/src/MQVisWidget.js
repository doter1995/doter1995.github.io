function MQVisWidget(config) {
  ...
  this.render = function () {
        ...
        .attr('y', function (d, i) {
          ...
          if(length>100){
            ...
            for(var i =1;i<steps-1;i++){
              ...
              d3.select(this.parentNode).append('text').text(Number(d.data[0]-d.range[0]+Y.invert(baseY-i*50)).toFixed(2))
              .attr('font-size', "10")
              .attr('font-family', "sans-serif")
              .attr('text-anchor', 'middle')
              .attr('class', 'text3')
    ...
    _zoomRoot.on('mouseout.1',MouseOut)
...
function zoomform(xz,yz,mouse){
  var xingtest = (new Date()).valueOf()
    ...
    var xingtest1 = (new Date()).valueOf()
    move(xd, yd, xz, yz)
    ...
    yrect.selectAll('text.text3').remove()
      ...
      .attr('y', function (d, i) {
        ...
        return ydata > 5 && ydata <= H + 2 ? ydata : -66
      })
    _yAxis.selectAll('g.ys').select('text.text2')
        ...
        if(length>100){
          ...
          for(var i =1;i<steps-1;i++){
            ...
            if(yvalue>0&&yvalue<H){
              d3.select(this.parentNode).append('text').text(Number(d.data[0]-d.range[0]+_yz.invert(baseY-i*50)).toFixed(2))
              .attr('font-size', "10")
              .attr('font-family', "sans-serif")
              .attr('text-anchor', 'middle')
              .attr('class', 'text3')
      ...
      .attr('y', function (d, i) {
        ...
        return ydata > 5 && ydata < H ? ydata : -66
      })
    //更新背景色
    _bg.selectAll('rect')
      .attr('y', function (d, i) {
  ...
  function move(xdate, ydate, x, y, preventEvent) {
    ...
    var linesShow = _Lines.selectAll('line.line')
      ...
      .filter(function (d) {
        d['isLight']={left:false,right:false}
        //第一遍过滤
        ...
        if (d1 < xdate && d2 > xdate) {
          ...
          if (mouse[1]<H-3&&yValueS - xhoverSelect < mouse[1] && yValueS + xhoverSelect > mouse[1]) {
            isshow = true
    ...
    linesShow
      .filter(function (d) {
        ...
        // if (d.isLight.left) {
        //   var show = false
        //   linesShow.filter(function(dd){
        //       return dd.isLight.right
        //     }).each(function (dd) {
        //     if (dd.date[0] >= d.date[1]) {
        //       show = true
        //       return true
        //     }
        //   })
        ...
        //       return d.isLight.left
        //     }).each(function (dd) {
        //     if (d.date[0] >= dd.date[1]) {
        //       show = true
        //       return true
        //     }
        //   })
      ...
      .attr('stroke', color1).raise()
      // console.log('testDate',new Date().valueOf()-testDate,OnHoverSelectData.length)
    _Lines.selectAll('line.line').filter(function (d) {
      //调用外部数据判断是否有线条需要高亮
      var re = false
      var data = d.data
      for(var i = 0, li = OnHoverSet.length; i < li; i++){
        var dd = OnHoverSet[i];
  ...
  function MouseOut(a){
      //清除y值重置为-1
      markerY=-1
      move(parseDate(markerX),-1 , _xz, _yz)
  }
